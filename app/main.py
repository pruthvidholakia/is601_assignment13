...
def update_calculation(
    ...
    new_type = calculation_update.type or calculation.type
    new_inputs = calculation_update.inputs or calculation.inputs

    # If type hasn't changed → update in place
    if new_type == calculation.type:
        if calculation_update.inputs is not None:
            calculation.inputs = new_inputs
            from sqlalchemy.orm.attributes import flag_modified
            flag_modified(calculation, 'inputs')  # Required for JSON updates
    ...
    # Else → type has changed → delete and recreate using proper subclass
    try:
